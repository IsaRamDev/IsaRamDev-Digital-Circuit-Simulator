...
const XnorGate = ({ id, startConnection, completeConnection}) => {
  const [inputA, setInputA] = useState(0);
  const [inputB, setInputB] = useState(0);
  const output = !((!inputA * inputB) + (inputA * !inputB));
  ...
  return (
    <div>
      <svg width="120" height="60" viewBox="0 0 120 60" xmlns="http://www.w3.org/2000/svg">
        <path d="M 20,30 L 48,30 M 20,50 L 48,50 M 30,20 Q 50,40 30,60 M 40,20 L 70,20 Q 100,40 70,59 L 40,59 M 85,40 L 100,40"
              stroke="black" fill="transparent" strokeWidth="2"/>
        <path d="M 40,20 Q 60,40 40,60"
              stroke="black" fill="transparent" strokeWidth="2"/>
        <circle cx="88" cy="40" r="4" fill="white" stroke="black"/>
        ...
        <text x="110" y="43" fontSize="10">{Number(output)}</text>
      </svg>
    </div>
  );
};
